...
exports.inject = ['@deepseek-ai/dsh-client-runtime', '@deepseek-ai/dsh-client-ui-slots', '@deepseek-ai/dsh-client-locale']
exports.apply = apply;
...
const USAGE_CSS = [
  '/* ---- 用量明细视图 ---- */',
  '.dsb-panel.dsb-panel-usage{width:680px;max-width:calc(100vw - 24px)}',
  '.dsb-u-filters{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:10px 14px;border-bottom:1px solid rgba(128,128,128,.16)}',
  '.dsb-u-seg{display:flex;border:1px solid rgba(128,128,128,.26);border-radius:8px;overflow:hidden}',
  '.dsb-u-seg button{border:none;background:transparent;color:var(--dsw-alias-label-secondary,#9a9a9a);font:inherit;font-size:12px;padding:4px 12px;cursor:pointer;line-height:1.4;transition:background .12s ease,color .12s ease}',
  ...
  '.dsb-u-dates{display:flex;align-items:center;gap:4px;margin-left:auto;color:var(--dsw-alias-label-secondary,#9a9a9a);font-size:11px}',
  '.dsb-u-date{border:1px solid rgba(128,128,128,.26);border-radius:6px;background:var(--dsw-alias-bg-inplace,#1d1d20);color:var(--dsw-alias-label-primary,#ececec);font:inherit;font-size:11px;padding:3px 6px;color-scheme:dark}',
  '.dsb-u-tablewrap{max-height:min(440px,calc(100vh - 230px));overflow:auto;margin:10px 12px 4px;border:1px solid rgba(128,128,128,.16);border-radius:10px}',
  '.dsb-u-table{width:100%;border-collapse:collapse;font-size:12px}',
  '.dsb-u-table th{position:sticky;top:0;background:var(--dsw-alias-bg-overlay,#2b2b2f);color:var(--dsw-alias-label-secondary,#9a9a9a);font-weight:500;text-align:left;padding:8px 12px;border-bottom:1px solid rgba(128,128,128,.2);white-space:nowrap;z-index:1}',
  '.dsb-u-table td{padding:8px 12px;border-bottom:1px solid rgba(128,128,128,.08);vertical-align:middle}',
  '.dsb-u-table tr:last-child td{border-bottom:none}',
  '.dsb-u-table tbody tr:hover{background:rgba(128,128,128,.09)}',
  '.dsb-u-time{white-space:nowrap;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;color:var(--dsw-alias-label-secondary,#9a9a9a)}',
  '.dsb-u-prompt{max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:default}',
  '.dsb-u-model{white-space:nowrap;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}',
  '.dsb-u-product{white-space:nowrap;color:var(--dsw-alias-label-secondary,#9a9a9a)}',
  ...
  '.dsb-u-foot{display:flex;justify-content:space-between;align-items:center;padding:8px 16px 12px;color:var(--dsw-alias-label-secondary,#9a9a9a);font-size:11px}',
  '.dsb-u-foot b{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;color:var(--dsw-alias-label-primary,#ececec)}',
  '.dsb-u-empty{padding:32px 0;text-align:center;color:var(--dsw-alias-label-secondary,#9a9a9a);font-size:12px}',
].join('')
...
function apply(ctx) {
  ...
  // 8.9 解析 locale：ctx.get 软获取，缺席降级内置 zh
  let locale = 'zh'
  try {
    const localeSvc = ctx.get('@deepseek-ai/dsh-client-locale')
    if (localeSvc && localeSvc.locale) locale = String(localeSvc.locale).startsWith('zh') ? 'zh' : 'en'
  } catch (e) { /* 忽略 */ }
