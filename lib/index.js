...
const USAGE_MAX_RECORDS = 2000 // 明细账本上限，超出裁剪最旧
const PROMPT_MAX_CHARS = 120 // 明细里单条提问截断长度（对齐 DSH CONTEXT_SUMMARY_MAX_CHARS）
const USAGE_DEFAULT_SPAN_MS = 7 * 24 * 3600 * 1000 // usage 查询缺省时间窗：7 天
...
function extractPrompt(messages) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const text = m.content
      ...
      .join('\n')
      .trim()
    if (text) return text.length > PROMPT_MAX_CHARS ? text.slice(0, PROMPT_MAX_CHARS - 1) + '…' : text
  }
  return ''
}

// 解析 usage 查询的 from/to（epoch ms，客户端按本地时区算好传上来）。
// 缺省回退最近 7 天；from >= to 时交换；非法值回退默认窗口。
function parseUsageRange(searchParams) {
  const now = Date.now()
  const defaultFrom = now - USAGE_DEFAULT_SPAN_MS
  const num = (v) => {
    const n = Number(v)
...
export async function apply(ctx) {
  ...
  // 一轮结束：把进行中的明细记录落账。费用口径 = 整轮全部模型调用（与面板「上轮花费」一致），
  // 含本轮内发生的辅助调用费用。
  ...
  if (llm !== undefined) {
    ctx.on('llm/stream', (options, next) => {
      const auxiliary = !!(options && options.purpose) // 压缩/会话标题等辅助调用：计费但不产生明细行
      if (!auxiliary) {
        ...
        if (state.ended) {
          ...
          pendingUsage = {
            ...
            models: [],
            prompt: extractPrompt(options && options.messages),
            calls: 0,
          }
        }
        if (pendingUsage) {
  ...
  if (webServer !== undefined) {
    ...
    const disposeUsage = webServer.register({
      ...
      handler: async (req, res) => {
        try {
          const url = new URL(req.url || '/', 'http://localhost')
          const { from, to } = parseUsageRange(url.searchParams)
          const records = (ledger.usage || [])
            .filter((r) => r && r.at >= from && r.at < to)
            .sort((a, b) => b.at - a.at)
          const total = records.reduce((s, r) => s + (Number(r.cost) || 0), 0)
          res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' })
          res.end(JSON.stringify({ records, total, count: records.length, currency: state.currency, at: Date.now() }))
        } catch (e) {
          res.writeHead(500, { 'Content-Type': 'application/json; charset=utf-8' })
          res.end(JSON.stringify({ error: errMsg(e) }))
        }
