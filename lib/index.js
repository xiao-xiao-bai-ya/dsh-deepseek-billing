...
export async function apply(ctx) {
  ...
  if (tools !== undefined) {
    tools.register(defineTool({
      ...
      description: '查询 DeepSeek API 账户余额与本地实时计费：返回当前余额(current)、上一轮结束时的余额(last)、本轮已花费(spent，llm/stream 实时按 token 计价)、当前对话框开始余额(dialogStartBalance)、币种(currency)。用户问「余额多少」「这次/上一轮花了多少钱」「对话框开始余额」「现在实时花了多少」时使用。输入无参数。',
      parameters: {},
