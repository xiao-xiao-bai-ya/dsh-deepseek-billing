...
export async function apply(ctx, config) {
  ...
  if (llm !== undefined) {
    ctx.on('llm/stream', (options, next) => {
      ...
      // 流启动即刷新活跃时间，避免 2s 定时器在“流已开始但首块还没到”的窗口里
      // 把刚建的空轮次提前结算成 0 花费（这是历史明细里 cost=0 的根因）。
  ...
  const setupTimers = (timerCtx = ctx) => {
    ...
    timerCtx.interval(() => { fetchBalance() }, 5000)
    timerCtx.timeout(() => { fetchBalance() }, 1000)
  }
  if (typeof ctx.inject === 'function') {
    // ctx.inject(['timer'], cb) 会为回调创建一个注入了 timer 服务的子上下文；
    // 定时器方法（interval/timeout）必须用这个子上下文的，外层 ctx 没有这些方法。
    ctx.inject(['timer'], (timerCtx) => setupTimers(timerCtx))
  } else {
    setupTimers()
  }
